...
### 2. プラグインのインストール（対話式）
推奨は対話式インストーラーです。`copy`（Windowsは`xcopy`）または`symlink`を選べます。

```bash
uv run python scripts/install_plugin.py
```

WindowsのPowerShell向けラッパー:
```powershell
...
Windowsで`symlink`を選択し、通常権限で作成できない場合は管理者PowerShellへの昇格確認が表示されます。
...
7. **保存**:
   - 出力フォルダを選択
   - ファイル名に含めるフィールドを選択（日付は必須、その他は任意）
   - 「Save Image」ボタンで保存

### 設定の永続化
Text Inserterは以下の設定を自動的に保存します：
- ファイル名フィールドの選択状態
- ウィンドウサイズ
- 最後に使用したCSVファイル
- 最後に使用した出力フォルダ
